...
// updated by DevHelper_Helper_ShippableHelper at 2016-05-25T09:07:35+00:00

/**
 * Class bdMails_ShippableHelper_UpdaterCore
 * @version 1
...
class bdMails_ShippableHelper_UpdaterCore
{
    ...
    {
        if (isset($GLOBALS[bdMails_ShippableHelper_Updater::KEY]['setupMethod'][$config['apiUrl']])) {
            if (XenForo_Application::debugMode()) {
                die(sprintf('Updater for %s has been setup by %s already!', $config['apiUrl'],
                    $GLOBALS[bdMails_ShippableHelper_Updater::KEY]['setupMethod'][$config['apiUrl']]));
            }
            return;
        }
        $GLOBALS[bdMails_ShippableHelper_Updater::KEY]['setupMethod'][$config['apiUrl']] = __METHOD__;
...
EOF;
        ...
        if (!empty($_GET[self::PARAM_DOWNLOAD])
            ...
            if (!empty($data[$addOnId]['links']['content'])) {
                $tempFile = bdMails_ShippableHelper_TempFile::download($data[$addOnId]['links']['content']);
    ...
    private static function _updateSelf(array $config)
    {
        ...
        $configOptionId = bdMails_ShippableHelper_Updater::getConfigOptionId($config['apiUrl']);
        ...
        if ($configVersion < bdMails_ShippableHelper_Updater::$_version) {
            ...
            $config['version'] = bdMails_ShippableHelper_Updater::$_version;
            $configUpdated[] = '$config["version"]';
        }

        $addOnIdsUpdated = false;
        foreach ($GLOBALS[bdMails_ShippableHelper_Updater::KEY][$config['apiUrl']]['addOns'] as $addOnId => $enabled) {
        ...
        if (count($configUpdated) > 0) {
            ...
            if (XenForo_Application::debugMode()) {
                XenForo_Helper_File::log(__CLASS__, sprintf('%s($apiUrl=%s): $configVersion=%d,'
                    ...
                    var_export(bdMails_ShippableHelper_Updater::$_version, true),
    ...
    {
        $configOptionId = bdMails_ShippableHelper_Updater::getConfigOptionId($apiUrl);
        $templateTitle = $configOptionId;
        $templateModKey = $configOptionId;
    ...
    private static function _refreshData($apiUrl, array $addOnIds, $forceRefresh)
    {
        ...
        if (empty($data)
            || $data['version'] < bdMails_ShippableHelper_Updater::$_version
            || $data['timestamp'] < XenForo_Application::$time - 86400
        ) {
            $data = array(
                'version' => bdMails_ShippableHelper_Updater::$_version,
                'timestamp' => time(),
    ...
    private static function _fetchData($url, array $addOnIds, $accessToken = '')
    {
        ...
        $url .= sprintf('%s_version=%d', ((strpos($url, '?') === false) ? '?' : '&'),
            bdMails_ShippableHelper_Updater::$_version);
    ...
    private static function _getCache($apiUrl)
    {
        ...
        $data = $dataRegistryModel->get(bdMails_ShippableHelper_Updater::KEY);
    ...
    private static function _setCache($apiUrl, array $thisData)
    {
        ...
        $dataRegistryModel->set(bdMails_ShippableHelper_Updater::KEY, $data);
        return true;
    }
